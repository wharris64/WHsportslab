...
## Hard Mode
Hard mode is not required and is to challenge you on concepts related to state. In hard mode, we want to modify the **Game** component to be the source of the truth for the games state. The **Game** component should keep track of how the game is played and it's progress. Currently, most of this is tracked in our **Team** component. Our game component will manage 2 teams and their stats during the game. To accomplish this we will be converting our **Team** components to stateless functional components and having the **Game** component pass the data they need to function as props from its state. This is concept is called [Lifting State Up](https://reactjs.org/docs/lifting-state-up.html). We are sharing state with by moving it up to the closest common ancestor of the components that need it.
...
* Since all the game data will be tracked by the **Game** component, the method used when pressing shoot should also be passed in as a prop to each team. This method needs to affect state in the right component so the game updates properly.
